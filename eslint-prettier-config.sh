...
select package_command_choices in "npm" "Yarn" "Yarn_Workspaces"  "Cancel"; do
  case $package_command_choices in
    ...
    Yarn_Workspaces ) pkg_cmd='yarn add -W'; break;;
    Cancel ) exit;;
  esac
done
echo
...
echo
$pkg_cmd -D eslint-config-airbnb eslint-plugin-jsx-a11y eslint-plugin-import eslint-plugin-react eslint-plugin-react-hooks babel-eslint
